...
| Fase 4 — Compras y gastos              | En curso     | Alta manual de facturas de proveedor con snapshot del emisor, control de duplicados, líneas fiscales, deducibilidad parcial, numeración interna correlativa y aprobación idempotente e inmutable. La aprobación publica el documento en el Tax Ledger de facturas recibidas y contabiliza compra, IVA deducible y proveedor dentro de la misma transacción. Los pagos salientes manuales son append-only, idempotentes, parciales o totales, protegen contra sobrepago y actualizan saldo y contabilidad atómicamente. Pendientes adjuntos, vencimientos, circuito multinivel, upload y OCR con revisión humana. |
| Fase 5 — Motor contable                | En curso     | Foundation implementada con plan PGC inicial, cuentas configurables, ejercicios, periodos mensuales, diario y líneas. Contabilización automática de ventas, rectificativas, compras, cobros de cliente y pagos a proveedor, más asientos manuales idempotentes, numeración correlativa por ejercicio, inmutabilidad posted, reversión, bloqueo de periodos, balance de sumas y saldos, diario/mayor exportable y conciliación bancaria uno-a-uno. Los informes JSON/CSV están acotados, ordenados de forma determinista y protegidos contra inyección de fórmulas. Todo ello está aislado por empresa y validado en PostgreSQL. Pendientes reglas configurables, PyG, balance y cierre. |
...
**Estado real:** En curso. Implementadas alta manual, validación de proveedor, protección frente a números duplicados, snapshots fiscales con porcentaje deducible, aprobación idempotente, numeración interna correlativa, inmutabilidad y publicación transaccional al Tax Ledger de facturas recibidas y al diario contable. Implementados también pagos salientes manuales append-only e idempotentes, con saldo parcial/final, protección concurrente frente al sobrepago y asiento automático de proveedor contra banco. Pendientes adjuntos, vencimientos, upload, OCR y revisión humana.
...
**Estado real:** En curso. Implementados plan contable inicial inspirado en PGC, cuentas configurables, ejercicios y periodos mensuales, diario equilibrado, contabilización automática de facturas de venta, rectificativas, compras con IVA parcialmente deducible, cobros de cliente y pagos a proveedor contra bancos, asiento manual idempotente, numeración anual serializada, reversión, bloqueo de periodos, balance de sumas y saldos, diario y mayor exportables en JSON/CSV, conciliación bancaria uno-a-uno, auditoría, permisos, RLS e invariantes de inmutabilidad/equilibrio en PostgreSQL. Los informes aplican rangos acotados, orden determinista, totales y protección contra inyección de fórmulas. El gate técnico mínimo de asiento inmutable, reversión y periodo protegido está cubierto por pruebas integrales; la fase permanece abierta hasta incorporar reglas configurables, PyG, balance y cierre básicos.
