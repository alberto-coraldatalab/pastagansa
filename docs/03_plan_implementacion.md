...
| Fase 4 — Compras y gastos              | En curso     | Alta manual de facturas de proveedor con snapshot del emisor, control de duplicados, líneas fiscales, deducibilidad parcial, numeración interna correlativa y aprobación idempotente e inmutable. La aprobación publica el documento en el Tax Ledger de facturas recibidas y contabiliza compra, IVA deducible y proveedor dentro de la misma transacción. Implementados vencimientos configurables con detección de mora y congelación al aprobar; los pagos salientes manuales son append-only, idempotentes, parciales o totales, se reparten por antigüedad y actualizan vencimientos, saldo y contabilidad atómicamente. Los adjuntos PDF/PNG/JPEG se guardan transaccionalmente, validan firma y tamaño, se deduplican, aíslan por tenant y congelan al aprobar. El circuito multinivel configura tramos por importe de uno a cinco aprobadores distintos, congela el documento pendiente, admite rechazo a borrador y solo numera/contabiliza con la aprobación final. Pendientes antivirus/almacenamiento de objetos y OCR con revisión humana. |
