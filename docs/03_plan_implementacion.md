...
| Hito U — Producto usable               | En curso     | Prioridad activa. Ya existe el workspace web integrado con registro/login/logout, renovación en cookies HTTP-only, contexto de empresa autorizado bajo RLS, shell responsive y estados básicos de carga/error. El siguiente gate es cliente → factura emitida → PDF desde navegador. Después llegarán compra con adjunto/OCR, cobros/pagos, datos demo, E2E de navegador, accesibilidad y staging observable. Detalle en `docs/05_plan_producto_usable.md`. |
