...
`/presupuestos` creates quotations with selectable commercial series, creates the
first series in the browser when needed, downloads the numbered PDF, records the
customer decision, and converts accepted quotations to linked invoice drafts.
...
Issued sales invoices expose the configured email capability and delivery history. When
SMTP is available, the UI preserves an idempotency key across retries and shows queue
state, attempts, errors, and delivery time. Without a provider it disables sending and
directs the user to the supported PDF download instead of creating a stuck delivery.
