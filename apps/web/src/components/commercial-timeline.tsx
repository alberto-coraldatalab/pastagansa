...
type CommercialEventType = "SENT" | "DELIVERY_FAILED" | "ACCEPTED" | "REJECTED" | "DISPUTED" | "PARTIALLY_PAID" | "PAID" | "PAYMENT_PROMISED" | "NOTE";
...
const labels: Record<CommercialEventType, string> = {
  ...
  DISPUTED: "En disputa", PARTIALLY_PAID: "Cobro parcial", PAID: "Cobrado", PAYMENT_PROMISED: "Promesa de pago", NOTE: "Nota interna",
};

export function CommercialTimeline({ endpoint, canManage }: { endpoint: string; canManage: boolean }) {
  const queryClient = useQueryClient();
  const [type, setType] = useState<"ACCEPTED" | "REJECTED" | "DISPUTED" | "PAYMENT_PROMISED" | "NOTE">("ACCEPTED");
