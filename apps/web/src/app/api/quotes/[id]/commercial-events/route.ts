...
async function forward(method: string, idValue: string, body?: unknown) {
  const id = z.uuid().safeParse(idValue); if (!id.success) return NextResponse.json({ error: "Presupuesto inválido." }, { status: 400 });
  try { const response = await tenantApiRequest(`/v1/quotes/${id.data}/commercial-events`, { method, ...(body ? { headers: { "content-type": "application/json" }, body: JSON.stringify(body) } : {}) }); const data = await response.json().catch(() => undefined); if (!response.ok) return NextResponse.json({ error: normalizeApiError(data) }, { status: response.status }); return NextResponse.json(data, { status: response.status }); }
