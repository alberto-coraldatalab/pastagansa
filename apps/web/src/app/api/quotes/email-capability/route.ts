...
export async function GET() {
  try {
    const response = await tenantApiRequest("/v1/quotes/email-capability");
    ...
    return NextResponse.json({ error: "No podemos comprobar la configuración de correo." }, { status: 503 });
  }
}
