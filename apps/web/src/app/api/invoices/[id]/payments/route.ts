...
const recordSchema = paymentInputSchema.extend({
...
export async function GET(
  ...
  return forward(`/v1/invoices/${id.data}/payments`);
}
...
export async function POST(
  ...
  const [id, input] = await Promise.all([
    ...
    recordSchema.safeParseAsync(await request.json().catch(() => undefined)),
  ]);
  if (!id.success || !input.success)
    return NextResponse.json(
      { error: "Revisa el importe, la fecha y el método de cobro." },
      { status: 400 },
    );
