...
export async function GET(request: NextRequest) {
  ...
  for (const name of ["search", "cursor", "limit"] as const) {
    const value = request.nextUrl.searchParams.get(name);
    if (value) params.set(name, value);
  }
  return forward(`/v1/catalog-items?${params.toString()}`);
}

export async function POST(request: Request) {
  const input = catalogInputSchema.safeParse(
    await request.json().catch(() => undefined),
  );
  if (!input.success)
    return NextResponse.json(
      { error: "Revisa los datos del producto o servicio antes de guardarlo." },
      { status: 400 },
    );
  return forward("/v1/catalog-items", {
    method: "POST",
