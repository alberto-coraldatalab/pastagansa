...
export class TenantGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    if (this.reflector && !this.reflector.getAllAndOverride<boolean>(TENANT_PROTECTED, [context.getHandler(), context.getClass()])) return true;
    ...
    if (!organizationId) throw new ForbiddenException('x-organization-id is required');
    ...
    if (!memberships.length) throw new ForbiddenException('No active membership for this tenant');

    request.tenant = { organizationId, companyId, userId: request.user.id, roleCodes: memberships.map(({ role }) => role.code) };
    return true;
  }
}
