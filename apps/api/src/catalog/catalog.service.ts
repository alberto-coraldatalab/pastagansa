...
export class CatalogService {
  ...
  async list(query: ListCatalogItemsDto) {
    ...
    const items = await this.tenant.db.catalogItem.findMany({
      where: {
        ...this.scope(),
    ...
    return {
      data,
      nextCursor:
        hasMore && last ? encodeCursor(last.id, last.name, search) : null,
    };
  }
  ...
  async create(input: CreateCatalogItemDto) {
    ...
    try {
      const item = await this.tenant.db.catalogItem.create({
        data: { ...this.scope(), ...normaliseCreate(input) },
      });
      await this.audit.record("catalog_item.created", "catalog_item", item.id, {
        type: item.type,
      });
      return item;
    } catch (error) {
  ...
  async importCsv(input: ImportCatalogItemsDto) {
    ...
    if (rows.length > 1_000)
      throw new BadRequestException(
        "A catalog import cannot contain more than 1,000 rows",
      );
    const scope = this.scope();
    const skus = rows.flatMap(({ sku }) => (sku ? [sku] : []));
    if (skus.length) {
      const existing = await this.tenant.db.catalogItem.findFirst({
        where: { ...scope, sku: { in: skus } },
        select: { sku: true },
      });
      if (existing?.sku)
        throw new ConflictException(
          `A catalog item with SKU ${existing.sku} already exists`,
        );
    }
    try {
      const items = await Promise.all(
        rows.map((row) =>
          this.tenant.db.catalogItem.create({
            data: {
              ...scope,
              type: row.type,
              sku: row.sku,
              ...
              suggestedTaxCode: row.suggestedTaxCode,
  ...
  async update(id: string, input: UpdateCatalogItemDto) {
    ...
    this.validateInventory(
      ...
      input.trackInventory ?? item.trackInventory,
    );
    try {
      const updated = await this.tenant.db.catalogItem.update({
        where: { id: item.id },
        data: normaliseUpdate(input),
      });
      await this.audit.record("catalog_item.updated", "catalog_item", item.id, {
  ...
  async archive(id: string) {
    const item = await this.findActive(id);
    const archived = await this.tenant.db.catalogItem.update({
      where: { id: item.id },
      data: { status: CatalogItemStatus.ARCHIVED },
    });
  ...
  private validateInventory(type: CatalogItemType, trackInventory: boolean) {
    if (type === CatalogItemType.SERVICE && trackInventory)
  ...
  private async findActive(id: string) {
    const item = await this.tenant.db.catalogItem.findFirst({
      where: { id, ...this.scope(), status: CatalogItemStatus.ACTIVE },
    });
  ...
  private async assertCursorInScope(id: string, sort: string) {
    const cursor = await this.tenant.db.catalogItem.findFirst({
      where: {
        id,
        name: sort,
        ...this.scope(),
...
function normaliseCreate(input: CreateCatalogItemDto) {
  return {
    ...
    suggestedTaxCode: blankToNull(input.suggestedTaxCode),
    revenueAccountCode: blankToNull(input.revenueAccountCode),
    expenseAccountCode: blankToNull(input.expenseAccountCode),
...
function normaliseUpdate(input: UpdateCatalogItemDto) {
  return {
    ...input,
    ...(input.sku !== undefined ? { sku: blankToNull(input.sku) } : {}),
    ...(input.name !== undefined ? { name: input.name.trim() } : {}),
    ...(input.description !== undefined
      ? { description: blankToNull(input.description) }
      : {}),
    ...(input.unit !== undefined ? { unit: input.unit.trim() || "unit" } : {}),
    ...(input.suggestedTaxCode !== undefined
      ? { suggestedTaxCode: blankToNull(input.suggestedTaxCode) }
      : {}),
    ...(input.revenueAccountCode !== undefined
      ? { revenueAccountCode: blankToNull(input.revenueAccountCode) }
      : {}),
    ...(input.expenseAccountCode !== undefined
      ? { expenseAccountCode: blankToNull(input.expenseAccountCode) }
      : {}),
  };
}
