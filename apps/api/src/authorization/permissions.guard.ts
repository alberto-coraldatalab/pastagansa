...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!request.tenant) throw new ForbiddenException('A tenant context is required');
    const matches = await this.prisma.rolePermission.findMany({
      where: { role: { code: { in: request.tenant.roleCodes } }, permission: { code: { in: required } } },
      select: { permission: { select: { code: true } } },
    });
    const granted = new Set(matches.map(({ permission }) => permission.code));
    if (required.some((permission) => !granted.has(permission))) throw new ForbiddenException('Missing required permission');
    return true;
  }
}
